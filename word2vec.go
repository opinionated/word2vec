...
type Model struct {
	...
	// trie to get better memory efficiency with a prefix tree (trie)
	trie art.Tree
}
...
func (m *Model) addWord(word string, vec Vector) {

	m.trie.Insert(art.Key(word), vec)
}

func (m *Model) getWord(word string) (Vector, bool) {
	i, ok := m.trie.Search(art.Key(word))
	if !ok {
		return nil, false
	}
...
func FromReader(r io.Reader) (*Model, error) {
	...
	// just for printing progress as we build
	every := 50000
	step := 0
	for i := 0; i < size; i++ {
		if i == step {
			fmt.Println("at step:", step)
			step += every
		}
